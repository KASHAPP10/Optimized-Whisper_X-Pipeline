#!/usr/bin/env python3
"""Evaluate generated CSV transcript against human CSV (00017.csv).
Produces WER and a basic alignment CSV for inspection.
"""
import argparse
import pandas as pd
import numpy as np
...
def read_transcript_csv(path):
    df = pd.read_csv(path)
    # Expect columns: text or Transcript or similar
...
def simple_wer(ref, hyp):
    # Word error rate via Levenshtein on tokens
    r = ref.split()
    h = hyp.split()
    import numpy as _np
...
def compare(refs, hyps):
    # align by index; produce per-segment WER and aggregate
    rows=[]
    total_err=0.0
    total_ref_words=0
...
def parse_args():
    ...
    p.add_argument('--ref', required=True, help='Path to human CSV (00017.csv)')
